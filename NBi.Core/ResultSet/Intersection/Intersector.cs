...
namespace NBi.Core.ResultSet.Intersection
{
    public abstract class Intersector
    {
        ...
        public Intersector(ISettingsResultSet settings)
            => Settings = settings;

        public ResultIntersectionRows Execute(object x, object y)
            => Compare(TypeToTable(x), TypeToTable(y));

        protected DataTable TypeToTable(object obj)
        {
            switch (obj)
            {
                case DataTable table: return table;
                case ResultSet rs: return rs.Table;
                default: throw new ArgumentException();
            }
        }

        protected virtual ResultIntersectionRows Compare(DataTable x, DataTable y)
        {
            ...
            stopWatch.Start();
            var dictX = new Dictionary<KeyCollection, int>();
            BuildRowDictionary(x, keyComparer, dictX);
            Trace.WriteLineIf(Extensibility.NBiTraceSwitch.TraceInfo, $"Building dictionary for first dataset: {x.Rows.Count} [{stopWatch.Elapsed:d\\d\\.hh\\h\\:mm\\m\\:ss\\s\\ \\+fff\\m\\s}]");
            var dictY = new Dictionary<KeyCollection, int>();
            BuildRowDictionary(y, keyComparer, dictY);
            Trace.WriteLineIf(Extensibility.NBiTraceSwitch.TraceInfo, $"Building dictionary for second dataset: {x.Rows.Count} [{stopWatch.Elapsed:d\\d\\.hh\\h\\:mm\\m\\:ss\\s\\ \\+fff\\m\\s}]");
            stopWatch.Reset();

            var intersectedRows = dictX.Keys.Intersect(dictY.Keys);

            if ((intersectedRows?.Count() ?? 0) == 0)
                return ResultIntersectionRows.Empty;
            else
                return new ResultIntersectionRows(intersectedRows);
        }
